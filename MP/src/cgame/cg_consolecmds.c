...
=============
*/
static void CG_FreecamUP(void) {
	VectorMA(cg.demoCamPos, cg.demoCamSpeed, cg.refdef.viewaxis[2], cg.demoCamPos);
}
static void CG_FreecamDOWN(void) {
	VectorMA(cg.demoCamPos, -cg.demoCamSpeed, cg.refdef.viewaxis[2], cg.demoCamPos);
}
static void CG_FreecamLEFT(void) {
	VectorMA(cg.demoCamPos, cg.demoCamSpeed, cg.refdef.viewaxis[1], cg.demoCamPos);
}
static void CG_FreecamRIGHT(void) {
	VectorMA(cg.demoCamPos, -cg.demoCamSpeed, cg.refdef.viewaxis[1], cg.demoCamPos);
}
...
=============
*/
static void CG_Freecamsetpos(void) {
...
=============
*/
static void Cmd_ExecAtTime_f(void) {
	char		buffer[MAX_TOKEN_CHARS];
	char*		execString;
	char		*execString2;
	int			execTime;
	...
	if (trap_Argc() < 2) {
		CG_Printf("usage: exec_at_time <time> <file>\n");
		CG_PriorityCenterPrint("usage: exec_at_time <time> <file>", SCREEN_HEIGHT * 0.85, 8 * 0.6, 1);
		return;
	}
	trap_Argv(1, buffer, sizeof(buffer));
	execTime = atof(buffer);

	if (execTime < cg.time) {
		CG_Printf("Woops this frame has passed... Maybe wrong Demo?\n");
		CG_PriorityCenterPrint("Woops this frame has passed... Maybe wrong Demo?", SCREEN_HEIGHT * 0.85, 8 * 0.6, 1);
		return;
	}

	trap_Argv(2, buffer, sizeof(buffer));
	execString = buffer;
	trap_Cvar_Set("mm_execAtTimeTime", va("%i\n", execTime));
	trap_Cvar_Set("mm_execAtTime", "1");
	trap_Cvar_Set("mm_execAtTimeVal", va("%s\n", execString));
	execString2 = va("%s \"^1%s^7\" %s \"^0%i^7\"", CG_TranslateString("Im gonna exec"), execString, CG_TranslateString("at Frame Number"), execTime);
	//CG_Printf(execString2);
	//CG_PriorityCenterPrint(execString2, SCREEN_HEIGHT * 0.85, 8 * 0.6, 1);
}
...
//mmMod - Freecam
static void CG_DemoCam(void) {
	cg.demoCam = cg.demoCam ? 0 : 1;
	cg.demoCamSpeed = mm_demoFreecamSpeed.value;
...
//	{ "camera", CG_Camera_f },	// duffy
	...
	{ "freecam", CG_DemoCam },
	{ "freecamUP", CG_FreecamUP },
	{ "freecamDOWN", CG_FreecamDOWN },
	{ "freecamLEFT", CG_FreecamLEFT },
	{ "freecamRIGHT", CG_FreecamRIGHT },
	{ "exec_at_time", Cmd_ExecAtTime_f },
