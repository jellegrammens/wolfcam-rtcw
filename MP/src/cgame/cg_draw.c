...
void CG_AddPMItem(const char* message) {
	vec4_t colourText = { 1.f, 1.f, 1.f, 1.f };
	int sizex = 8, sizey = 20;
		int size;
...
void CG_DrawUpperRight( void ) {
    ...
    //CG_Printf ("cg_draw.c:CG_DrawUpperRight\n");
...
typedef struct {
	...
	int     snapshotEstimate[LAG_SAMPLES];
	int		snapshotCount;
...
void CG_AddLagometerSnapshotInfo( snapshot_t *snap ) {
	...
	// dropped packet
	if ( !snap ) {
		lagometer.snapshotSamples[ lagometer.snapshotCount & ( LAG_SAMPLES - 1) ] = -1000;
		lagometer.snapshotCount++;
		return;
	}

	if (snap->ps.clientNum != cg.clientNum)
		return;

	if (lagometer.count % (10 * 20) == 0) {
        lagometer.count = 0;
        lagometer.total = 0;
        lagometer.pingTotal = 0;
    }

	if (cg.snap) {
		//lagometer.snapshotEstimate[lagometer.snapshotCount & (LAG_SAMPLES - 1) ] = cg.snap->serverTime - snap->ps.commandTime - (snap->serverTime - cg.snap->serverTime) / 2;
		lagometer.snapshotEstimate[lagometer.snapshotCount & (LAG_SAMPLES - 1) ] = cg.snap->serverTime - snap->ps.commandTime - 20;  // why 20?  (snap->serverTime - cg.snap->serverTime) / 2;
		//lagometer.snapshotEstimate[lagometer.snapshotCount & (LAG_SAMPLES - 1)] = cg.snap->serverTime - snap->ps.commandTime;
		lagometer.count++;
		lagometer.total += lagometer.snapshotEstimate[lagometer.snapshotCount & (LAG_SAMPLES - 1) ];
    ...
    if (cg.demoPlayback  &&  snap->ps.pm_type != PM_INTERMISSION  && snap->ps.clientNum == cg.clientNum) {
		snap->ping = lagometer.snapshotEstimate[lagometer.snapshotCount & (LAG_SAMPLES - 1)];
    }

	lagometer.pingTotal += snap->ping;
...
static void CG_DrawDisconnect( void ) {
    ...
    if (cg.demoPlayback  &&  cg_timescale.integer < 1.0)
        return;
...
//	if(0) {
	...
	//CG_Printf ("%d %d %d\n", totalSum / i, i, totalSum);

	//CG_DrawBigString (x, y + 20, va("%d", totalSum / count), 1.0);
	//CG_DrawBigString (x, y + 20, va("%d  %d", totalSum / i, i), 1.0);
	if (lagometer.count) 
		CG_DrawBigString (x, y + 20, va("%d", lagometer.pingTotal / lagometer.count), 1.0);
...
static void CG_DrawCrosshair(void) {
	...
	if ( cg_crosshairHealth.integer ) {
        ...
        //CG_Printf ("%f %f %f\n", cg.xhairColor[0], cg.xhairColor[1], cg.xhairColor[2]);
...
=================
*/
void CG_ActivateLimboMenu( void ) {
...
static void CG_Draw2D( void ) {
	...
	if (cg_draw2D.integer == 0) {
		if (mm_demoCrosshair.integer) CG_DrawCrosshair();
	...
	if (mm_blood.integer) CG_DrawFlashBlendBehindHUD();
	...
	if (mm_blood.integer) CG_DrawFlashBlend();
}
