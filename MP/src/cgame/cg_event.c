...
static void CG_Obituary( entityState_t *ent ) {
	...
	// Get protocol info for demo playback
	const char  *info;
	int p;
	...
	p = atoi(Info_ValueForKey(info, "protocol"));
	...
	//debug for dm_59 demos
	if (p == 59 && target == 0 && attacker == 0) {
		return;
	}
    ...
    if (wolfcam_following)
        clientNum = cgs.clientinfo[wcg.clientNum].clientNum;
    else
...
// JPW NERVE per atvi req
		...
		case MOD_DYNAMITE_SPLASH:
			if (mm_vapYourselfe.integer) message = "dynamited himself to pieces";
            wclients[attacker].wstats[WP_DYNAMITE].suicides++;
			break;
// jpw
		case MOD_GRENADE_SPLASH:
			if (mm_vapYourselfe.integer) message = "dove on his own grenade";
            wclients[attacker].wstats[WP_GRENADE_PINEAPPLE].suicides++;
			break;
		case MOD_ROCKET_SPLASH:
			if (mm_vapYourselfe.integer) message = "vaporized himself";
            wclients[attacker].wstats[WP_PANZERFAUST].suicides++;
			break;
		case MOD_AIRSTRIKE:
			if (mm_vapYourselfe.integer) message = "obliterated himself";
            wclients[attacker].wstats[WP_ARTY].suicides++;
			break;
			//case MOD_BFG_SPLASH:
			//message = "should have used a smaller gun";
			//break;
		case MOD_EXPLOSIVE:
			if (mm_vapYourselfe.integer) message = "died in his own explosion";
			break;
		default:
    ...
    {
        switch (mod) {
        case MOD_KNIFE_STEALTH:  //FIXME wc not different weap though
        case MOD_KNIFE:
        case MOD_KNIFE2:
            wclients[attacker].wstats[WP_KNIFE].kills++;
            wclients[target].wstats[WP_KNIFE].deaths++;
            break;
        case MOD_LUGER:
            wclients[attacker].wstats[WP_LUGER].kills++;
            wclients[target].wstats[WP_LUGER].deaths++;
            break;
        case MOD_COLT:
            wclients[attacker].wstats[WP_COLT].kills++;
            wclients[target].wstats[WP_COLT].deaths++;
            break;
        case MOD_MP40:
            wclients[attacker].wstats[WP_MP40].kills++;
            wclients[target].wstats[WP_MP40].deaths++;
            break;
        case MOD_THOMPSON:
            wclients[attacker].wstats[WP_THOMPSON].kills++;
            wclients[target].wstats[WP_THOMPSON].deaths++;
            break;
        case MOD_STEN:
            wclients[attacker].wstats[WP_STEN].kills++;
            wclients[target].wstats[WP_STEN].deaths++;
            break;
        case MOD_MAUSER: 
            wclients[attacker].wstats[WP_MAUSER].kills++;
            wclients[target].wstats[WP_MAUSER].deaths++;
            break;
        case MOD_SNIPERRIFLE:
            wclients[attacker].wstats[WP_SNIPERRIFLE].kills++;
            wclients[target].wstats[WP_SNIPERRIFLE].deaths++;
            break;
        case MOD_DYNAMITE:
        case MOD_DYNAMITE_SPLASH:
            wclients[attacker].wstats[WP_DYNAMITE].kills++;
            wclients[target].wstats[WP_DYNAMITE].deaths++;
            break;
        case MOD_ROCKET_LAUNCHER:
        case MOD_ROCKET_SPLASH:
            wclients[attacker].wstats[WP_PANZERFAUST].kills++;
            wclients[target].wstats[WP_PANZERFAUST].deaths++;
            break;
        case MOD_GRENADE_LAUNCHER:
        case MOD_GRENADE_SPLASH:
        case MOD_GRENADE_PINEAPPLE:
            wclients[attacker].wstats[WP_GRENADE_PINEAPPLE].kills++;
            wclients[target].wstats[WP_GRENADE_PINEAPPLE].deaths++;
            break;
        case MOD_VENOM:
            wclients[attacker].wstats[WP_VENOM].kills++;
            wclients[target].wstats[WP_VENOM].deaths++;
            break;
        case MOD_VENOM_FULL:
            wclients[attacker].wstats[WP_VENOM].kills++;
            wclients[target].wstats[WP_VENOM].deaths++;
        ...
        case MOD_FLAMETHROWER:
            wclients[attacker].wstats[WP_FLAMETHROWER].kills++;
            wclients[target].wstats[WP_FLAMETHROWER].deaths++;
            break;
        case MOD_MACHINEGUN:
            wclients[attacker].wstats[WOLFCAM_WP_MG42].kills++;
            wclients[target].wstats[WOLFCAM_WP_MG42].deaths++;
            break;
        case MOD_AIRSTRIKE:
            wclients[attacker].wstats[WP_ARTY].kills++;
            wclients[target].wstats[WP_ARTY].deaths++;
            break;
        default:
            break;
        }
	...
	if (message) {
		...
		if (mm_drawYourOwnFragsOnly.integer && killtype == 2)
		{
			message3 = va("%s %s.\n", targetName, message);
			CG_AddPMItem(message3);
			//CG_Printf( "[cgnotify]%s %s.\n", targetName, message);
		}
		else
			if (!mm_drawYourOwnFragsOnly.integer)
			{
				//CG_Printf( "[cgnotify]%s %s.\n", targetName, message);
				message3 = va("%s %s.\n", targetName, message);
				CG_AddPMItem(message3);
	...
	if ( attacker == clientNum && !cg.demoCam) {
		char	*s;
		...
		if (ci->team == ca->team && mm_teamkills.integer == 0) {

		}
		else {
			if (mm_freecamConsole.integer == 2 || mm_freecamConsole.integer == 0) CG_PriorityCenterPrint(s, SCREEN_HEIGHT * 0.75, BIGCHAR_WIDTH * 0.6, 1);
		}
...
// JPW NERVE if attacker != target but on same team
		if (ci->team == ca->team) {
			if (mm_teamkills.integer) message = "^1WAS KILLED BY TEAMMATE^7";
			message2="";
		}
// jpw
		...
		if (message) {
			...
			if (mm_drawYourOwnFragsOnly.integer && (clientNum == attacker))
			{
	...
	switch (mod) {
	case MOD_MACHINEGUN:
		message3 = va("%s was riddled by machinegun fire\n", targetName);
		CG_AddPMItem(message3);
		break;
	default:
		message3 = va("%s died.\n", targetName);
...
//----(SA)	end
	...
	case EV_FALL_SHORT:
		...
		if ( !wolfcam_following  &&  clientNum == cg.predictedPlayerState.clientNum ) {
			// smooth landing z changes
			cg.landChange = -8;
			cg.landTime = cg.time;
        }
        if (es->number >= 0  &&  es->number < MAX_CLIENTS) {
            wclients[es->number].landChange = -8;
            wclients[es->number].landTime = cg.time;
        }
		break;
/*
	case EV_FALL_MEDIUM:
...
*/
	case EV_FALL_DMG_10:
        ...
        if (es->number >= 0  &&  es->number < MAX_CLIENTS) {
            wclients[es->number].landChange = -16;
            wclients[es->number].landTime = cg.time;

            //FIXME wolfcam EV_PAIN sync
            wclients[es->number].ev_pain_time = cg.time;
            wclients[es->number].eventHealth -= 10;
        }
		break;
	case EV_FALL_DMG_15:
		DEBUGNAME("EV_FALL_DMG_15");
        //CG_Printf ("EV_FALL_DMG_15: %d\n", es->number);
		// use normal pain sound trap_S_StartSound( NULL, es->number, CHAN_VOICE, CG_CustomSound( es->number, "*pain100_1.wav" ) );
		trap_S_StartSound (NULL, es->number, CHAN_AUTO, CG_CustomSound( es->number, "sound/multiplayer/land_hurt.wav" ) ); // JPW NERVE
		cent->pe.painTime = cg.time;	// don't play a pain sound right after this
		if ( !wolfcam_following  &&  clientNum == cg.predictedPlayerState.clientNum ) {
        ...
        if (es->number >= 0  &&  es->number < MAX_CLIENTS) {
            ...
            //FIXME wolfcam EV_PAIN sync .. DamageFeedback() ?
            wclients[es->number].eventHealth -= 15;
            wclients[es->number].ev_pain_time = cg.time;
        }
		break;
	case EV_FALL_DMG_25:
		DEBUGNAME("EV_FALL_DMG_25");
        //        CG_Printf ("EV_FALL_DMG_25: %d\n", es->number);
		trap_S_StartSound (NULL, es->number, CHAN_AUTO, CG_CustomSound( es->number, "sound/multiplayer/land_hurt.wav" ) ); // JPW NERVE
		cent->pe.painTime = cg.time;	// don't play a pain sound right after this
		if ( !wolfcam_following  &&  clientNum == cg.predictedPlayerState.clientNum ) {
			// smooth landing z changes
        ...
        if (es->number >= 0  &&  es->number < MAX_CLIENTS) {
            wclients[es->number].landChange = -24;
            wclients[es->number].landTime = cg.time;

            //FIXME wolfcam EV_PAIN sync .. DamageFeedback() ?
            wclients[es->number].eventHealth -= 25;
            wclients[es->number].ev_pain_time = cg.time;
        }
		break;
/*
...
*/
	case EV_FALL_DMG_50:
        ...
        if (es->number >= 0  &&  es->number < MAX_CLIENTS) {
            ...
            //FIXME wolfcam EV_PAIN sync .. DamageFeedback() ?
            wclients[es->number].eventHealth -= 50;
            wclients[es->number].ev_pain_time = cg.time;
        }
		break;
/*
...
*/	
		...
		DEBUGNAME("EV_STEP");
	{
        ...
        // wolfcam so stepTime not needed
		if (wolfcam_following  ||  clientNum != cg.predictedPlayerState.clientNum) {
			break;
		}
...
//			trap_R_SetFog(FOG_WATER, 0, 400, .1, .1, .1, 111);
	...
	case EV_ITEM_PICKUP_QUIET:
		DEBUGNAME("EV_ITEM_PICKUP");
		{
            ...
            if (wolfcam_following  &&  es->number == cgs.clientinfo[wcg.clientNum].clientNum)  //FIXME wolfcam double check
                CG_ItemPickup (index);
...
//----(SA)	draw the HUD items for a sec since this is a special item
            ...
            if (item->giType == IT_HEALTH  &&  es->number < MAX_CLIENTS)
                wclients[es->number].eventHealth += 20;
		}
		break;
	...
	case EV_GLOBAL_ITEM_PICKUP:
		DEBUGNAME("EV_GLOBAL_ITEM_PICKUP");
		{
            ...
            trap_S_StartSound (NULL, es->number, CHAN_AUTO, trap_S_RegisterSound(item->pickup_sound));  // wolfcam

			// show icon and name on status bar
			if (!wolfcam_following  &&   es->number == cg.snap->ps.clientNum) {
				CG_ItemPickup( index );
			} else if (wolfcam_following  &&  es->number == cgs.clientinfo[wcg.clientNum].clientNum) {
                CG_ItemPickup (index);
            }
	...
	case EV_WEAP_OVERHEAT:
        ...
        if (es->number >= 0  &&  es->number < MAX_CLIENTS) {
            wclients[es->number].weapAnim = ( (wclients[es->number].weapAnim & ANIM_TOGGLEBIT ) ^ ANIM_TOGGLEBIT ) | WEAP_IDLE1;
            wclients[es->number].event_overheatTime = cg.time;
        }
...
// jpw
	...
	case EV_FILL_CLIP:
        ...
        if (es->number >= 0  &&  es->number < MAX_CLIENTS)
            wclients[es->number].event_reloadTime = cg.time;
...
// jpw
	...
	case EV_CHANGE_WEAPON:
        ...
        if (es->number >= 0  &&  es->number < MAX_CLIENTS) {
            wclients[es->number].event_weaponSwitchTime = cg.time;
            wclients[es->number].event_newWeapon = 0;
            wclients[es->number].event_oldWeapon = es->weapon;
        }
...
// jpw
	...
	case EV_FIRE_WEAPON:
// JPW NERVE
		if (!wolfcam_following  &&  cg.snap->ps.eFlags & EF_ZOOMING) // to stop airstrike sfx
			break;
// jpw
	case EV_FIRE_WEAPONB:
		DEBUGNAME("EV_FIRE_WEAPON");
        if (cent->currentState.clientNum >= 0  &&  cent->currentState.clientNum < MAX_CLIENTS)
            wclients[cent->currentState.clientNum].event_fireWeaponTime = cg.time;  //FIXME wolfcam this ok ?
		CG_FireWeapon( cent);
		break;
...
// jpw
	...
	case EV_MISSILE_HIT:
        ...
        //wdebug ("^1xxx  EV_MISSILE_HIT  %d -> %d\n", es->number, cent->currentState.number);
        //break;
        //FIXME wolfcam damage?
        if (es->otherEntityNum < MAX_CLIENTS) {  // wolfcam testing
            //CG_Printf ("^3missile hit %s\n", cgs.clientinfo[es->otherEntityNum].name);
#if 0  //FIXME wolfcam do you even need?
            if (cg.time - wclients[es->otherEntityNum].ev_pain_time < PAIN_EV_TIME  &&
                cg.time - wclients[es->otherEntityNum].ev_pain_time != 0) {

                wclients[es->otherEntityNum].eventHealth -= 20;  //FIXME wolfcam who knows
            }
#endif
        }

        //wdebug ("cent:%p, es->weapon:%d, es->otherEntityNum:%d\n", cent, es->weapon, es->otherEntityNum);
		CG_MissileHitPlayer( cent, es->weapon, position, dir, es->otherEntityNum );
        if (cent->currentState.number < MAX_CLIENTS) {
            //wdebug ("EV_MISSILE_HIT  %d -> %d\n", es->number, cent->currentState.number);
            wclients[es->number].wstats[es->weapon].hits++;
        }
...
#if 0
        CG_Printf ("EV_MG42BULLET_HIT_FLESH: clientNum:%d  otherEntityNum:%d  eventParm:%d  otherEntityNum2:%d\n",
                   clientNum, es->otherEntityNum, es->eventParm,
                   es->otherEntityNum2);
#endif
        if (es->otherEntityNum2 >= 0  &&  es->otherEntityNum2 < MAX_CLIENTS)
            wclients[es->otherEntityNum2].wstats[WOLFCAM_WP_MG42].hits++;
		break;
...
//		CG_MachineGunEjectBrass(cent); // JPW NERVE
	...
	case EV_GLOBAL_SOUND:	// play from the player's head so it never diminishes
		...
		// done.
        {
            ...
            if (!wolfcam_following)
                ...
                cn = cgs.clientinfo[wcg.clientNum].clientNum;

		if ( cgs.gameSounds[ es->eventParm ] ) {
			trap_S_StartSound (NULL, cn, CHAN_AUTO, cgs.gameSounds[ es->eventParm ] );
		} else {
			s = CG_ConfigString( CS_SOUNDS + es->eventParm );
			trap_S_StartSound (NULL, cn, CHAN_AUTO, CG_CustomSound( es->number, s ) );
		}
        }
		break;
	...
	case EV_GLOBAL_CLIENT_SOUND:
		...
		if (!wolfcam_following  &&  cg.snap->ps.clientNum == es->teamNum ) {
			if ( cgs.gameSounds[ es->eventParm ] ) {
				trap_S_StartSound (NULL, cg.snap->ps.clientNum, CHAN_AUTO, cgs.gameSounds[ es->eventParm ] );
			}
		}

        if (wolfcam_following  &&  cgs.clientinfo[wcg.clientNum].team == es->teamNum) { //FIXME wolfcam-rtcw
            if ( cgs.gameSounds[ es->eventParm ] ) {
                trap_S_StartSound (NULL, cg.snap->ps.clientNum, CHAN_AUTO, cgs.gameSounds[ es->eventParm ] );
            }
        }
	...
	case EV_PAIN:
		...
		DEBUGNAME("EV_PAIN");
        wclients[cent->currentState.number].ev_pain_time = cg.time;

#if 0
        if (wolfcam_following  &&  cent->currentState.number == cgs.clientinfo[wcg.clientNum].clientNum) {
            //wolfcam_health = es->eventParm;
            wclients[wcg.clientNum].eventHealth = es->eventParm;
            //CG_Printf ("^3PAIN!!\n");
        }
#endif
        ...
        if (cent->currentState.number < MAX_CLIENTS) {
            wclients[cent->currentState.number].ev_pain_time = cg.time;
            wclients[cent->currentState.number].eventHealth = es->eventParm;
        }
	...
	case EV_CROUCH_PAIN:
        ...
        //FIXME wolfcam double check this, looks wierd
		if (!wolfcam_following  &&  cent->currentState.number != cg.snap->ps.clientNum ) {
        ...
        if (wolfcam_following  &&  cent->currentState.number != cgs.clientinfo[wcg.clientNum].clientNum)
            CG_PainEvent (cent, es->eventParm, qtrue);
		if (p <= 58) {
			CG_Obituary(es);  // added for 1.0 - 1.31 demos
		}
		break;
	...
	case EV_DEATH2:
		if (p == 59) {
			CG_Obituary(es);  // added for 1.33 demos
		}
	case EV_DEATH3:
		DEBUGNAME("EV_DEATHx");
        ...
        if (es->number < MAX_CLIENTS)
            wclients[es->number].eventHealth = 0;
		break;
