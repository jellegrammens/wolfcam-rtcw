...
// Freecam 
vmCvar_t	mm_demoFreecamSpeed;
vmCvar_t	mm_demoCrosshair;
vmCvar_t	mm_demoPopUp;
vmCvar_t	mm_drawYourOwnFragsOnly;
vmCvar_t	mm_execAtTime;
vmCvar_t	mm_execAtTimeTime;
vmCvar_t	mm_execAtTimeVal;
vmCvar_t	mm_teamkills;
vmCvar_t	mm_vapYourselfe;
vmCvar_t	mm_blood;
...
vmCvar_t	mm_seperateNotifyQueue;
...
//	{ &cg_smoothClients, "cg_smoothClients", "0", CVAR_USERINFO | CVAR_ARCHIVE},
    ...
    // wolfcam-rtcw
    { &wolfcam_debugErrors, "wolfcam_debugErrors", "0", CVAR_ARCHIVE },
    { &wolfcam_debugDrawGun, "wolfcam_debugDrawGun", "0", CVAR_ARCHIVE },
    { &wolfcam_fakelag, "wolfcam_fakelag", "0", CVAR_ARCHIVE },
    { &wolfcam_fast_forward_timescale, "wolfcam_fast_forward_timescale", "8", CVAR_ARCHIVE },
    { &wolfcam_fast_forward_invalid_time, "wolfcam_fast_forward_invalid_time", "2.0", CVAR_ARCHIVE },
    { &wolfcam_avoidDemoPov, "wolfcam_avoidDemoPov", "1", CVAR_ARCHIVE },
    { &wolfcam_tryToStickWithTeam, "wolfcam_tryToStickWithTeam", "1", CVAR_ARCHIVE },
	{ &wolfcam_warp_shader_time, "wolfcam_warp_shader_time", "0", CVAR_ARCHIVE },
	{ &wolfcam_wh, "wolfcam_wh", "0", CVAR_ARCHIVE },

	{ &mm_demoFreecamSpeed, "mm_demoFreecamSpeed", "20", CVAR_ARCHIVE },
	{ &mm_demoCrosshair, "mm_demoCrosshair", "1", CVAR_ARCHIVE },
	{ &mm_demoPopUp, "mm_demoPopUp", "1", CVAR_ARCHIVE },
	{ &mm_drawYourOwnFragsOnly, "mm_drawYourOwnFragsOnly", "0", CVAR_ARCHIVE },
		// exec_at_time
	{ &mm_execAtTime, "mm_execAtTime", "0", CVAR_ARCHIVE },
	{ &mm_execAtTimeTime, "mm_execAtTimeTime", "0", CVAR_ARCHIVE },
	{ &mm_execAtTimeVal, "mm_execAtTimeVal", "", CVAR_ARCHIVE },
	{ &mm_teamkills, "mm_drawTeamkills", "1", CVAR_ARCHIVE },
	{ &mm_vapYourselfe, "mm_vapYourselfe", "1", CVAR_ARCHIVE },
	{ &mm_blood, "mm_bloodflash", "0", CVAR_ARCHIVE },
	...
	{ &mm_seperateNotifyQueue, "mm_seperateNotifyQueue", "0", CVAR_ARCHIVE }
};
...
static colorTable_t OSP_Colortable[] =
    {
        { "white",              
          &colorWhite },
        { "red",                
...
static void BG_setCrosshair(char *colString, float *col, float alpha, char *cvarName)
{
    char *s = colString;
    ...
    if(*s == '0' && (*(s+1) == 'x' || *(s+1) == 'X')) {
        ...
        if(Q_IsHexColorString(s)) {
            col[0] = ((float)(gethex(*(s)) * 16 + gethex(*(s+1)))) /
                255.00;
            col[1] = ((float)(gethex(*(s+2)) * 16 + gethex(*(s+3))))
                / 255.00;
            col[2] = ((float)(gethex(*(s+4)) * 16 + gethex(*(s+5))))
                / 255.00;
            return;
        }
    } else {
...
void CG_UpdateCvars( void ) {
	...
	for ( i = 0, cv = cvarTable ; i < cvarTableSize ; i++, cv++ ) {
        ...
        if(cv->vmCvar == &cg_crosshairColor || cv->vmCvar == &cg_crosshairAlpha) {
            BG_setCrosshair(cg_crosshairColor.string, cg.xhairColor, cg_crosshairAlpha.value, "cg_crosshairColor");
            //CG_Printf ("^3 setting crosshair color to %s\n", cg_crosshairColor.string);
        }
...
//----(SA)	end
	...
	for ( i = 0 ; i < NUM_CROSSHAIRS ; i++ ) {
		cgs.media.crosshairShader[i] = trap_R_RegisterShader( va("gfx/2d/crosshair%c_w", 'a'+i) );
		cg.crosshairShaderAlt[i] = trap_R_RegisterShader( va("gfx/2d/crosshair%c_alt_w", 'a'+i) );
	}
