...
static void CG_ConfigStringModified( void ) {
    ...
    //CG_Printf ("CG_ConfigStringModified: %03d  %s %s\n", num, Wolfcam_ConfigStringName(num), str);
	...
	if ( num == CS_MUSIC ) {
		...
		CG_ParseWolfinfo();
        //FIXME wolfcam
		//CG_Printf ("xxxxxxxxxxxxxxxx    cgs.gamestate: %d\n", cgs.gamestate);
		if (cgs.gamestate == GS_INTERMISSION)
			Wolfcam_Weapon_Statsall_f ();
	} else if ( num == CS_SCORES1 ) {
		cgs.scores1 = atoi( str );
        //CG_Printf ("xxxx scores1  %s\n", str);
	} else if ( num == CS_SCORES2 ) {
		cgs.scores2 = atoi( str );
        //CG_Printf ("xxxx scores2  %s\n", str);
        ...
        //FIXME wolfcam not send in shrub
        //Wolfcam_Weapon_Statsall_f ();
...
//----(SA)	
        ...
        //CG_Printf ("^3CG_ConfigStringModified:  unknown config string number %d %s\n", num, str);
    }
}
...
static void CG_ServerCommand( void ) {
	...
	if ( !strcmp( cmd, "scores" ) ) {
        //CG_Printf ("^1---------------------------------------parse scores\n");
        //CG_Printf ("CG_ServerCommand:  scores\n");
		CG_ParseScores();
        Wolfcam_ScoreData ();
		return;
	}
	...
	if ( !strcmp( cmd, "cp" ) ) {
		...
		if ( args >= 3 ) {
			...
			if (!cg.demoCam) CG_PriorityCenterPrint( s, SCREEN_HEIGHT - (SCREEN_HEIGHT * 0.25), SMALLCHAR_WIDTH, atoi( CG_Argv( 2 ) ) );
		}
		else {
			if (!cg.demoCam) CG_CenterPrint( CG_LocalizeServerCommand( CG_Argv(1) ), SCREEN_HEIGHT - (SCREEN_HEIGHT * 0.25), SMALLCHAR_WIDTH );	//----(SA)	modified
		}
		return;
	}
	...
	if ( !strcmp( cmd, "print" ) ) {
		if (mm_wtvchat.integer) {
			if (mm_seperateNotifyQueue.integer) {
				CG_Printf("[cgnotfI]1%s", CG_LocalizeServerCommand(CG_Argv(1)));
			}
			else {
				CG_Printf("[cgnotify]1%s", CG_LocalizeServerCommand(CG_Argv(1)));
			}
		}
#ifdef MISSIONPACK
