...
void CG_ZoomIn_f( void ) {
    if (wolfcam_following && !cg.demoCam) {
        Wolfcam_ZoomIn ();
        return;
    }
...
void CG_DamageBlendBlob( void ) {
	...
	for (i=0; i<MAX_VIEWDAMAGE; i++) {
        ...
        // wolfcam testing
        //CG_Printf ("CG_DamageBlendBlob: duration:%d  cg.time:%d time:%d  damageX:%f  damageY:%f  value:%f\n", vd->damageDuration, cg.time, vd->damageTime, vd->damageX, vd->damageY, vd->damageValue);
        ...
        //CG_Printf ("real blob added to scene\n");
		redFlash += ent.radius;
	}
...
void CG_DrawActiveFrame( int serverTime, stereoFrame_t stereoView, qboolean demoPlayback ) {
	...
	if (mm_execAtTime.integer == 1) {
		if (mm_execAtTimeTime.integer <= cg.time) {
			trap_SendConsoleCommand(va("exec %s\n", mm_execAtTimeVal.string));
			trap_Cvar_Set("mm_execAtTimeTime", "0");
			trap_Cvar_Set("mm_execAtTime", "0");
			trap_Cvar_Set("mm_execAtTimeVal", "-");
		}
	}
...
//----(SA)	nerve uses this for snooper/sniper
	...
	//mmMod - Freecam
		if (cg.demoCam) 	cg.renderingThirdPerson = 1;
    ...
    //FIXME wolfcam shit, needs to be done after entities lerped
#if 0
    if (wolfcam_following) {
        Wolfcam_TransitionPlayerState ();
        if (cg_entities[cgs.clientinfo[wcg.clientNum].clientNum].currentState.eFlags & EF_DEAD)
	...
	if (!wolfcam_following  &&  !cg.renderingThirdPerson && mm_blood.integer) {
		CG_DamageBlendBlob();
	} else if (wolfcam_following && !cg.demoCam && !(cg_entities[cgs.clientinfo[wcg.clientNum].clientNum].currentState.eFlags & EF_DEAD)) {
	...
	if ( !cg.hyperspace ) {
        //Wolfcam_MarkValidEntities ();  // do before AddPacketEnt so you can set cent->currentState.eFlags
    ...
    if (wolfcam_following && !cg.demoCam) {
        ...
        if (cg_entities[cgs.clientinfo[wcg.clientNum].clientNum].currentState.eFlags & EF_DEAD)
            ...
            Wolfcam_OffsetFirstPersonView ();
            if (mm_blood.integer) Wolfcam_DamageBlendBlob ();
        }
        inwater = Wolfcam_CalcFov ();
    }

	//if follow is used and freecam at same time
	if (cg.demoCam && wolfcam_following) {
		float fov = cg_fov.value;
		float x;
		VectorCopy(cg.demoCamPos, cg.refdef.vieworg);
		VectorCopy(cg.demoCamAngles, cg.refdefViewAngles);
    ...
    //FIXME wolfcam problem if it's here. no blood blobs, and things like dynamic lights are based on demo taker's pov (can be seen through walls)
#if 0
    if (wolfcam_following) {
        Wolfcam_TransitionPlayerState ();
        if (cg_entities[cgs.clientinfo[wcg.clientNum].clientNum].currentState.eFlags & EF_DEAD)
    ...
    if (wolfcam_following && !cg.demoCam) {
        if (wcg.clientNum == cg.clientNum  &&  wolfcam_debugDrawGun.integer)
            CG_AddViewWeapon( &cg.predictedPlayerState );
...
*/
	...
	if (mm_blood.integer) CG_DrawScreenFade();
    ...
    if (!wolfcam_following) {
        ...
        cgDC.ownerDrawItem = &Wolfcam_OwnerDraw;
        Wolfcam_DrawActive (stereoView);
    }
    ...
    if (wolfcam_following && !cg.demoCam)
        trap_S_Respatialize (cgs.clientinfo[wcg.clientNum].clientNum, cg.refdef.vieworg, cg.refdef.viewaxis, inwater);
    else
