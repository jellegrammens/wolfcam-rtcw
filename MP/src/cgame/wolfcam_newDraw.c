...
extern int CG_DrawField (int x, int y, int width, int value, int charWidth, int charHeight, qboolean dodrawpic, qboolean leftAlign );		// NERVE - SMF
extern int CG_GetSelectedPlayer (void);
extern void CG_DrawMedal(int ownerDraw, rectDef_t *rect, float scale, vec4_t color, qhandle_t shader);
...
==============
*/
static void Wolfcam_DrawPlayerWeaponIcon( rectDef_t *rect, qboolean drawHighlighted, int align ) {
	int			size;
...
// jpw
    ...
    grenadeTimeLeft = -(cg.time - wc->grenadeDynoTime) + 4000;

	// pulsing grenade icon to help the player 'count' in their head
	if(grenadeTimeLeft  &&  (realweap == WP_DYNAMITE  ||  realweap == WP_DYNAMITE2  ||  realweap == WP_GRENADE_LAUNCHER  ||  realweap == WP_GRENADE_PINEAPPLE) &&  es->eFlags & EF_FIRING) {	// grenades and dynamite set this

		// these time differently 
		if(realweap == WP_DYNAMITE || realweap == WP_DYNAMITE2) {
			if(((wc->grenadeDynoLastTime)%1000)>((grenadeTimeLeft)%1000)) {
				trap_S_StartLocalSound(cgs.media.grenadePulseSound4, CHAN_LOCAL_SOUND );
			}
		} else {
			if(((wc->grenadeDynoLastTime)%1000)<((grenadeTimeLeft)%1000)) {
				switch( grenadeTimeLeft/1000) {
					case 3:
						trap_S_StartLocalSound(cgs.media.grenadePulseSound4, CHAN_LOCAL_SOUND );
						break;
					case 2:
		...
		scale = (float)((grenadeTimeLeft)%1000) / 100.0f;
		halfScale = scale * 0.5f;

		wc->grenadeDynoLastTime = grenadeTimeLeft;
	}
	else {
		scale = halfScale = 0;
	}
...
==============
*/
static void Wolfcam_DrawPlayerAmmoIcon( rectDef_t *rect, qboolean draw2D ) {
	centity_t	*cent;
	playerState_t	*ps;
	vec3_t		angles;
	vec3_t		origin;
...
==============
*/
extern int CG_DrawFieldWidth (int x, int y, int width, int value, int charWidth, int charHeight );

static void Wolfcam_DrawPlayerAmmoValue(rectDef_t *rect, float scale, vec4_t color, qhandle_t shader, int textStyle, int type) {
...
//		case WP_DYNAMITE: // JPW NERVE these recharge
    ...
    clipvalue = 0;  //FIXME wolfcam-rtcw
	if(special) {
	...
	if ( ammovalue > -1 && type == 0 ) {
		if (0) {  //(shader) {  //FIXME wolfcam-rtcw check
			trap_R_SetColor( color );
			CG_DrawPic(rect->x, rect->y, rect->w, rect->h, shader);
			trap_R_SetColor( NULL );
		} else {
...
static void Wolfcam_DrawPlayerLocation( rectDef_t *rect, float scale, vec4_t color, int textStyle  ) {
    //	clientInfo_t *ci = &cgs.clientinfo[cg.snap->ps.clientNum];
...
static void Wolfcam_DrawPlayerScore( rectDef_t *rect, float scale, vec4_t color, qhandle_t shader, int textStyle ) {
	char num[16];
    //	int value = cg.snap->ps.persistant[PERS_SCORE];
    int value = 0;  //FIXME wolfcam-rtcw
...
static void Wolfcam_DrawPlayerItem( rectDef_t *rect, float scale, qboolean draw2D) {
	int		value;
...
static void Wolfcam_DrawPlayerHealth(rectDef_t *rect, float scale, vec4_t color, qhandle_t shader, int textStyle ) {
	playerState_t	*ps;
	int value;
	vec4_t	color2;
    vec4_t wcolor;
    ...
    //value = cgs.clientinfo[wcg.clientNum].health;
    value = wclients[wcg.clientNum].eventHealth;
	...
	if (0) {  //(shader) {  // wolfcam
		...
		//if ( cgs.clientinfo[wcg.clientNum].powerups & (1 << PW_INVULNERABLE) ) {
        if (cg_entities[cgs.clientinfo[wcg.clientNum].clientNum].currentState.powerups & (1 << PW_INVULNERABLE)) {
...
static void Wolfcam_DrawTeamColor(rectDef_t *rect, vec4_t color) {
...
// wolfcam never use?
float Wolfcam_GetValue(int ownerDraw, int type) {
	centity_t	*cent;
 	clientInfo_t *ci;
	playerState_t	*ps;
...
#define SPRINTTIME 20000.0f
	...
	case CG_STAMINA:	//----(SA)	added
        return 0;  //FIXME wolfcam-rtcw check
		if(type == RANGETYPE_RELATIVE) {
...
//FIXME wolfcam-rtcw  what is this shit, never used
qboolean Wolfcam_OwnerDrawVisible(int flags) {
...
const char *Wolfcam_GetKillerText() {
    //FIXME wolfcam-rtcw
	const char *s = "";
	if ( cg.killerName[0] ) {
...
static void CG_DrawKiller(rectDef_t *rect, float scale, vec4_t color, qhandle_t shader, int textStyle ) {
	...
	if ( cg.killerName[0] ) {
	  ...
	  CG_Text_Paint(x - CG_Text_Width(Wolfcam_GetKillerText(), scale, 0) / 2, rect->y + rect->h, scale, color, Wolfcam_GetKillerText(), 0, 0, textStyle);
	}
...
const char *Wolfcam_GetGameStatusText() {
    //FIXME wolfcam-rtcw
	const char *s = "";
	if ( cgs.gametype < GT_TEAM) {
...
static void CG_DrawGameStatus(rectDef_t *rect, float scale, vec4_t color, qhandle_t shader, int textStyle ) {
	CG_Text_Paint(rect->x, rect->y + rect->h, scale, color, Wolfcam_GetGameStatusText(), 0, 0, textStyle);
}
...
//FIXME wolfcam-rtcw
void Wolfcam_DrawNewTeamInfo(rectDef_t *rect, float text_x, float text_y, float scale, vec4_t color, qhandle_t shader) {
#ifdef MISSIONPACK
	int xx;
	float y;
	int i, j, len, count;
...
void Wolfcam_DrawTeamSpectators(rectDef_t *rect, float scale, vec4_t color, qhandle_t shader) {
    //FIXME wolfcam-rtcw check
	if (cg.spectatorLen) {
		float maxX;
...
==============
*/
void Wolfcam_DrawWeapStability(rectDef_t *rect, vec4_t color, int align) {
...
==============
*/
void Wolfcam_DrawWeapHeat(rectDef_t *rect, int align) {
...
//	omBarFrac = 1.0f-barFrac;
    ...
    CG_FilledBar(rect->x, rect->y+6, rect->w, rect->h*0.82f, wcolor, wcolor, wcolor,  0.0 /*(float)cg.snap->ps.sprintTime / SPRINTTIME*/ , flags);

	// fill in the left side of the bar with the counter for the nofatigue powerup

	if(0) {  //(cg.snap->ps.powerups[PW_NOFATIGUE]) {  // wolfcam
...
==============
*/
void Wolfcam_OwnerDraw(float x, float y, float w, float h, float text_x, float text_y, int ownerDraw, int ownerDrawFlags, int align, float special, float scale, vec4_t color, qhandle_t shader, int textStyle) {
	rectDef_t rect;
  ...
  switch (ownerDraw) {
	case CG_PLAYER_WEAPON_ICON2D:
		Wolfcam_DrawPlayerWeaponIcon(&rect, ownerDrawFlags & CG_SHOW_HIGHLIGHTED, align);
		break;
	...
	case CG_PLAYER_AMMO_ICON:
		Wolfcam_DrawPlayerAmmoIcon(&rect, ownerDrawFlags & CG_SHOW_2DONLY);
		break;
	case CG_PLAYER_AMMO_ICON2D:
		Wolfcam_DrawPlayerAmmoIcon(&rect, qtrue);
		break;
	case CG_PLAYER_AMMO_VALUE:
		Wolfcam_DrawPlayerAmmoValue(&rect, scale, color, shader, textStyle, 0);
		break;
	case CG_CURSORHINT:
		CG_DrawCursorhint(&rect);
		break;
	case CG_PLAYER_AMMOCLIP_VALUE:
		Wolfcam_DrawPlayerAmmoValue(&rect, scale, color, shader, textStyle, 1);
		break;
	...
	case CG_PLAYER_WEAPON_HEAT:
		Wolfcam_DrawWeapHeat(&rect, align);
		break;
	case CG_PLAYER_WEAPON_STABILITY:
		Wolfcam_DrawWeapStability(&rect, color, align);
		break;
	case CG_STAMINA:
	...
	case CG_PLAYER_HOLDABLE:
		...
		break;
	case CG_PLAYER_ITEM:
		Wolfcam_DrawPlayerItem(&rect, scale, ownerDrawFlags & CG_SHOW_2DONLY);
		break;
	case CG_PLAYER_SCORE:
		Wolfcam_DrawPlayerScore(&rect, scale, color, shader, textStyle);
		break;
	case CG_PLAYER_HEALTH:
		Wolfcam_DrawPlayerHealth(&rect, scale, color, shader, textStyle);
		break;
	case CG_RED_SCORE:
	...
	case CG_PLAYER_LOCATION:
		Wolfcam_DrawPlayerLocation(&rect, scale, color, textStyle);
		break;
	case CG_TEAM_COLOR:
		Wolfcam_DrawTeamColor(&rect, color);
		break;
	case CG_CTF_POWERUP:
	...
	case CG_CAPTURES:
		...
		break;
	case CG_SPECTATORS:
		Wolfcam_DrawTeamSpectators(&rect, scale, color, shader);
		break;
	case CG_TEAMINFO:
		if (cg_currentSelectedPlayer.integer == numSortedTeamPlayers) {
			Wolfcam_DrawNewTeamInfo(&rect, text_x, text_y, scale, color, shader);
		}
		break;
...
void Wolfcam_MouseEvent(int x, int y) {
	int n;
...
void Wolfcam_GetTeamColor(vec4_t *color) {
    //FIXME wolfcam-rtcw
